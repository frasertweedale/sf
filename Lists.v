...
Theorem add_increases_count_by_one : forall (v : nat) (s : bag),
...
Theorem rev_snoc_is_cons_rev : forall (h : nat) (t : natlist),
  rev (snoc t h) = h :: rev t.
Proof.
  intros h t. induction t.
    reflexivity.
...
Proof.
  intros l. induction l.
  reflexivity.
  simpl. rewrite -> rev_snoc_is_cons_rev, IHl. reflexivity.
  Qed.
...
Proof.
  intros l1 l2 l3 l4. rewrite app_assoc, app_assoc. reflexivity. Qed.
...
Theorem snoc_app_is_app_snoc : forall (l1 l2 : natlist) (n : nat),
  snoc (l1 ++ l2) n = l1 ++ snoc l2 n.
Proof.
  intros l1 l2 n. destruct l2.
    simpl. rewrite snoc_append, app_nil_end. reflexivity.
    destruct l2.
      simpl. rewrite -> snoc_append, app_assoc. reflexivity.
      simpl. rewrite -> snoc_append, snoc_append.
...
Proof.
  intros l1 l2. induction l1.
    ...
    simpl. rewrite -> IHl1, snoc_app_is_app_snoc. reflexivity.
  Qed.
...
Theorem app_cons_snoc : forall (xs ys : natlist) (n : nat),
  xs ++ (n :: ys) = (snoc xs n) ++ ys.
Proof.
  intros xs ys n.
  induction xs.
    reflexivity.
...
Theorem count_sum_nil_r : forall (n : nat) (s : bag),
  count n s = count n (sum s []).
Proof.
  intros n s.
  assert (sum_nil_r : sum s [] = s).
    induction s. reflexivity. simpl. rewrite IHs. reflexivity.
  rewrite sum_nil_r.
  reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros l1 l2 H.
  rewrite <- rev_involutive, <- H, rev_involutive. reflexivity.
  Qed.
(** [] *)
...
Proof.
  destruct l. reflexivity. reflexivity. Qed.
(** [] *)
